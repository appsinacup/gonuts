...
extern PtyProxy *create_proxy(TermRenderer *renderer, const char *shell_path = nullptr);
...
void GDTerm::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("start_with_shell", "shell_path"), &GDTerm::start_with_shell);
	...
	ClassDB::bind_method(D_METHOD("select_all"), &GDTerm::select_all);
	...
	ClassDB::bind_method(D_METHOD("_on_context_menu_item_selected", "id"), &GDTerm::_on_context_menu_item_selected);
	...
	ADD_SIGNAL(MethodInfo("size_changed")); // Signal when terminal size changes
}
...
GDTerm::GDTerm() {
	...
	set_mouse_filter(Control::MOUSE_FILTER_PASS); // Enable mouse input including wheel events
	...
	_rows = 32; // Increased from 30 to 32 to ensure nsnake compatibility (needs 24+)
	_cols = 100; // Increased from 80 to 100 for better compatibility
...
int GDTerm::get_cols() const {
	return _cols;
}

int GDTerm::get_rows() const {
	return _rows;
}

void GDTerm::set_scroll_pos(int pos) {
	int max_scroll = (int)_scrollback.size();
...
void GDTerm::_ready() {
	// Enable internal processing for screen updates
	set_process_mode(Node::PROCESS_MODE_ALWAYS);
	set_process_internal(true); // Enable internal processing for _notification(NOTIFICATION_INTERNAL_PROCESS)
	...
	// Trigger initial draw to ensure content is visible
...
void GDTerm::set_vt_handler_log_path(String log_path) {
	if (vt_handler_log_path != log_path) {
		...
		if (vt_handler_log_path.length() != 0) {
			...
			_vt_handler_input_log->open(os_path.utf8().get_data(), std::ios::out);
		}
	}
}
...
void GDTerm::start_with_shell(const String &shell_path) {
	...
	// Create a new proxy with custom shell path
	CharString shell_utf8 = shell_path.utf8();
	const char *shell_cstr = shell_path.is_empty() ? nullptr : shell_utf8.get_data();
	_proxy = create_proxy(this, shell_cstr);
...
void GDTerm::_draw() {
	...
	// Prevent drawing during resize operations
	if (_font_space_size.x <= 0 || _font_space_size.y <= 0) {
		return;
	}
...
void GDTerm::gui_input(const Ref<InputEvent> &p_event) {
	...
	// Additional safety check - ensure we're properly initialized
	if (!is_inside_tree()) {
		return;
	}
	...
	if (ke.is_valid()) {
		if (ke->is_pressed()) {
			...
			// Additional safety checks for key input
			...
			if (_is_copy_request(code)) {
			...
			} else if (_is_select_all_request(code)) {
				select_all();
				...
				// Protect against buffer overflow and invalid proxy
				char buffer[100];
				memset(buffer, 0, sizeof(buffer)); // Clear buffer

				// Additional null check for proxy before using it
				if (_proxy != nullptr) {
					fill_term_string(buffer, sizeof(buffer) - 1, unicode, code, _send_alt_meta_as_escape);
	...
	if (mbe.is_valid()) {
		// Safety check: ensure we can access mouse button properties
		MouseButton button_index = mbe->get_button_index();
		if (button_index == MouseButton::LEFT) {
			if (mbe->is_pressed()) {
				// Safety check for font size
				if (_font_space_size.y <= 0 || _font_space_size.x <= 0) {
					return;
				}
				Vector2 mpos = mbe->get_position();
				// Adjust for font ascent since drawing starts at font_ascent
				float font_ascent = font->get_ascent(font_size);
				float adjusted_y = mpos.y - font_ascent;
				int mouse_row = adjusted_y / _font_space_size.y;
				int mouse_col = mpos.x / _font_space_size.x;

				// Add bounds checking for array access
				int target_line_index = mouse_row + _scrollback_pos;
				int total_lines = _scrollback.size() + _screen_lines.size();
				if (target_line_index < 0 || target_line_index >= total_lines) {
					return; // Invalid line index, ignore the click
				}

				if (!_selecting) {
					const GDTermLine &line = _get_term_line(target_line_index);
					if (_selection_active && !_is_in_selection(line.selectable_length, target_line_index, mouse_col)) {
						if (!mbe->is_shift_pressed()) {
							_selection_active = false;
							_selecting = false;
							_select_mode = SELECT_MODE_CHAR;
							_select_start_row = target_line_index;
				...
				_selecting = true;
				_select_end_row = target_line_index;
			...
			if (mbe->is_pressed()) {
				...
				Vector2 global_pos = mbe->get_global_position();
				_show_context_menu(global_pos);
			}
		} else if (button_index == MouseButton::WHEEL_UP) {
			// Handle mouse wheel scroll up
			if (mbe->is_pressed()) {
				int current_pos = get_scroll_pos();
				set_scroll_pos(current_pos - 3); // Scroll up by 3 lines
				Control::accept_event();
			}
			...
			if (mbe->is_pressed()) {
				int current_pos = get_scroll_pos();
				set_scroll_pos(current_pos + 3); // Scroll down by 3 lines
				Control::accept_event();
			}
		}
	}
	...
	if (mme.is_valid()) {
		if (_selecting && _font_space_size.y > 0 && _font_space_size.x > 0) {
			...
			// Calculate target row with bounds checking
			int target_row = _scrollback_pos + mouse_row;
			int total_lines = _screen_lines.size() + _scrollback.size();
...
GDTerm::_get_term_line(int row) const {
	if (row < 0) {
		// Return a safe default - the first scrollback line or screen line
		if (!_scrollback.empty()) {
			return _scrollback[0];
		} else if (!_screen_lines.empty()) {
			return _screen_lines[0];
		}
		// This should never happen if we have proper bounds checking elsewhere
		static const GDTermLine empty_line = GDTermLine();
		return empty_line;
	}
	...
	int screen_index = row - _scrollback.size();
	if (screen_index >= 0 && screen_index < (int)_screen_lines.size()) {
		return _screen_lines[screen_index];
	}

	// Out of bounds - return the last available line
	if (!_screen_lines.empty()) {
		return _screen_lines[_screen_lines.size() - 1];
	} else if (!_scrollback.empty()) {
		return _scrollback[_scrollback.size() - 1];
	}
...
void GDTerm::select_all() {
	// Select all content from the beginning of scrollback to the end of current screen
	_selection_active = true;
	_select_start_row = 0;
	_select_start_col = 0;

	// End at the last line with content
	_select_end_row = (int)_scrollback.size() + _rows - 1;
	_select_end_col = _cols;
...
void GDTerm::_make_pending_active() {
	...
	if (_pending_dirty) {
		bool should_follow = (_scrollback_pos >= (int)_scrollback.size()) || (_scrollback.size() == 0);
	...
	if (_pending_cursor_dirty) {
		...
		if (_cursor_displayed && (_scrollback.size() == 0 || _scrollback_pos >= (int)_scrollback.size())) {
			_scrollback_pos = _scrollback.size();
			call_deferred("_notify_scrollback");
		}
	}
...
bool GDTerm::_is_select_all_request(Key code) {
	Key key = code & KeyModifierMask::CODE_MASK;
#ifdef USE_COMMAND_KEY_FOR_SHORTCUT
	Key meta = code & KeyModifierMask::META;
	if (meta != Key::NONE && (key == Key::A)) {
		return true;
	}
#else
	Key ctrl = code & KeyModifierMask::CTRL;
	if (ctrl != Key::NONE && (key == Key::A)) {
		return true;
	}
#endif
	return false;
...
void GDTerm::_do_resize() {
	...
	// More accurate height calculation - the drawing starts at font_ascent
	// and each line takes font_height, so we need: font_ascent + (num_rows * font_height) <= size.y
	float font_height = font->get_height(font_size);
	float font_ascent = font->get_ascent(font_size);
	...
	if ((_rows != num_rows) || (_cols != num_cols)) {
		...
		_resize_pty();
		emit_signal("size_changed"); // Notify about size change
	}
	queue_redraw();
}
...
void GDTerm::_show_context_menu(const Vector2 &p_position) {
	// Make sure we have a valid parent before creating the popup
	Node *parent_node = get_parent();
	if (parent_node == nullptr) {
		print_error("GDTerm: Cannot show context menu - no parent node found");
		return;
	}

	// Make sure we're in the scene tree
	if (!is_inside_tree()) {
		print_error("GDTerm: Cannot show context menu - not inside scene tree");
		return;
	}
	...
	if (popup == nullptr) {
		print_error("GDTerm: Failed to create PopupMenu");
		return;
	}

	// Add menu items
	popup->add_item("Copy", 0);
	popup->add_item("Paste", 1);
	popup->add_separator();
	popup->add_item("Select All", 2);

	// Connect the item selected signal - use a more robust approach
	Error err = popup->connect("id_pressed", callable_mp(this, &GDTerm::_on_context_menu_item_selected));
	if (err != OK) {
		print_error("GDTerm: Failed to connect context menu signal");
		popup->queue_free();
		return;
	}
	...
	popup->set_position(p_position);
	popup->popup();

	// Auto-remove after use - use a safer approach
	popup->connect("popup_hide", Callable(popup, "queue_free"), CONNECT_ONE_SHOT);
}

void GDTerm::_on_context_menu_item_selected(int p_id) {
	switch (p_id) {
		case 0: // Copy
			emit_signal("copy_request");
			break;
